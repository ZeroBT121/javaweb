...
public class RegisteServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(StringUtil.isEmpty(username)||StringUtil.isEmpty(password)||StringUtil.isEmpty(rePassword)){
            ...
            request.setAttribute("result",resultDO.getInfo());
            request.getRequestDispatcher(PathConstence.JSP_WEB_BASE+"/registe.jsp").forward(request, response);
            return;
        ...
        try {
            ...
            resultDO.setInfo("注册成功,请重新登录！");
            request.setAttribute("result",resultDO.getInfo());
            request.getRequestDispatcher(PathConstence.JSP_WEB_BASE+"/login.jsp").forward(request, response);
            //request.getRequestDispatcher(PathConstence.W_SERVLET_BASE+"/getAllAuction").forward(request, response);
